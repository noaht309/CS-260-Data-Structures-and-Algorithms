...
void HashTable::PrintAll() {
	...
	//If not UINT_MAX output the key and bid data
	for(auto nodeSearch = nodes.begin(); nodeSearch != nodes.end(); ++nodeSearch){
		if(nodeSearch->key != UINT_MAX){
			cout << "Key " << nodeSearch->key << ": " << nodeSearch->bid.bidId << " | " << nodeSearch->bid.title << " | " << nodeSearch->bid.amount << " | " << nodeSearch->bid.fund << endl;

			//while node not nullptr... output node data
